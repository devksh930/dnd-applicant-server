...
class ApplicantTest {
	...
	@Test
	void isAdmissionResultViewable_메서드는_연결된_Event가_조회_가능_기간일_때_true를_반환한다() {
		final Applicant applicant = ApplicantFixture.create();
		final LocalDateTime now = LocalDateTime.now();
		applicant.withEvent(EventFixture.viewabelEvent(now));

		assertThat(applicant.isAdmissionResultViewable(now)).isTrue();
	}

	@Test
	void isAdmissionResultViewable_메서드는_연결된_Event가_조회_가능_기간_전일_때_false를_반환한다() {
		final Applicant applicant = ApplicantFixture.create();
		final LocalDateTime now = LocalDateTime.now();
		applicant.withEvent(EventFixture.notViewableEventBefore(now));

		assertThat(applicant.isAdmissionResultViewable(now)).isFalse();
	}

	@Test
	void isAdmissionResultViewable_메서드는_연결된_Event가_조회_가능_기간_후일_때_false를_반환한다() {
		final Applicant applicant = ApplicantFixture.create();
		LocalDateTime now = LocalDateTime.now();
		applicant.withEvent(EventFixture.notViewableEventAfter(now));
